...
export function markComplete(id: string, dateISO: string): void {
  ...
  if (!h) return
  if (!h.completedDates.some((d) => d.startsWith(dateISO.split("T")[0]))) {
...
/**
 * Removes a completion for a specific habit on a specific day
 */
export function removeCompletion(id: string, dateKey: string): void {
  const habit = findHabitById(id)
  if (!habit) return

  const [day, month, year] = dateKey.split("-").map(Number)

  habit.completedDates = habit.completedDates.filter((dateStr) => {
...
export function calculateStreak(habit: Habit): number {
  ...
  const completionDates = habit.completedDates.map((d) => new Date(d)).sort((a, b) => b.getTime() - a.getTime())
